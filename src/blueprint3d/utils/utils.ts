...
export class Utils {
  ...
  public static pointDistanceFromLine(x: number, y: number, x1: number, y1: number, x2: number, y2: number): number {
    if (!x || !y || !x1 || !y1 || !x2 || !y2) {
      console.error(`pointDistanceFromLine 输入异常`);
    }
    var tPoint = Utils.closestPointOnLine(x, y, x1, y1, x2, y2);
    // console.error(tPoint);
  ...
  static closestPointOnLine(x: number, y: number, x1: number, y1: number, x2: number, y2: number): { x: number, y: number } {
    if (!x || !y || !x1 || !y1 || !x2 || !y2) {
      // console.error(`closestPointOnLine输入异常`);
    }
  ...
  static isClockwise(points: any): boolean {
    // make positive
    let tSubX = Math.min(0, Math.min.apply(null, Utils.map(points, function (p: { x: any; }) {
      return p.x;
    })))
    let tSubY = Math.min(0, Math.min.apply(null, Utils.map(points, function (p: { x: any; }) {
      return p.x;
    })))
  ...
  static lineLineIntersect(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, x4: number, y4: number): boolean {
    function tCCW(p1: { x: any; y: any; }, p2: { x: any; y: any; }, p3: { x: any; y: any; }) {
      var tA = p1.x,
        tB = p1.y,
        tC = p2.x,
        tD = p2.y,
  ...
  */
  ...
  static polygonInsidePolygon(insideCorners: { y: number; }[] | any, outsideCorners: any[], startX: number, startY: number): boolean {
    startX = startX || 0;
    startY = startY || 0;
  ...
  static polygonOutsidePolygon(insideCorners: { y: number; }[] | any, outsideCorners: any[], startX: number, startY: number): boolean {
    startX = startX || 0;
    startY = startY || 0;
  ...
  static forEach(array: any[], action: (arg0: any) => void) {
    for (var tI = 0; tI < array.length; tI++) {
      action(array[tI]);
    }
  }

  static forEachIndexed(array: any[], action: (arg0: number, arg1: any) => void) {
    for (var tI = 0; tI < array.length; tI++) {
      action(tI, array[tI]);
    }
  }
   ...
   * 如果func（element）返回true，则删除数组中的元素
   */
  ...
  static cycle(arr: { slice: (arg0: number) => void; }, shift: number) {
    var tReturn: any = arr.slice(0);
  ...
  static unique(arr: any[], hashFunc: (arg0: any) => string | number) {
    var tResults = [];
    var tMap = {};
  ...
  static subtract(array: { forEach: (arg0: (element: any) => void) => void; }, subArray: any[]) {
